# Boundary reproduction through actual saved receiver input; no worker launched.
# Desired outcome: every accepted completed journal in this bounded fixture can
# produce its automatic report within the existing worker transport contract.
source("R/platform-load.R", encoding = "UTF-8"); brohn_load(ui = FALSE)
local({
  store <- brohn_open_store(tempfile("brohn-revision-boundaries-"))
  on.exit({for (job in brohn_list_jobs(store)) if (job$status == "queued") brohn_cancel_job(store, job$id); brohn_close_store(store)}, add = TRUE)
  ...
  job <- brohn_list_jobs(store)[[1L]]; input <- brohn_job_input(store, job)
  input_file <- tempfile(fileext = ".json"); brohn_write_json_file(input, input_file)
  report <- brohn_analyse_runs(input)
  envelope <- list(schema = "brohn-analysis-output/1.0", code_identity = list(), report = report)
  sizes <- list(question_count = 200L, entered_answer_count = 400L, text_characters = nchar(text),
    final_record_count = length(report$analysis$questionnaire_revision$runs[[1L]]$effective_records),
    input_bytes = file.info(input_file)$size, output_bytes = nchar(brohn_json(envelope), type = "bytes"), worker_limit_bytes = 16*1024^2,
    full_paging_rows = length(recovered), workspace = store$root, scientific_workers_run = 0L)
  print(sizes)
  directory <- "../../work/test-runs/brohn-questionnaire-revision-boundary"
  dir.create(directory, recursive = TRUE, showWarnings = FALSE)
  brohn_write_json_file(sizes, file.path(directory, "sizes.json"))
  brohn_write_json_file(envelope, tempfile(fileext = ".json"))
  cat("PASS: actual accepted questionnaire history and canonical report fit their worker transport.\n")
})
