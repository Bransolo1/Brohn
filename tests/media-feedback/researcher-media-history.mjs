...
const output=path.join(folder,`browser-${Date.now()}`);await fs.mkdir(output);await fs.mkdir(path.join(output,'executed-harness'));for(const name of ['researcher-media-history.mjs','researcher-media-history-extension.mjs','researcher-media-history-fixture.R','browser-fixture.py'])await fs.copyFile(path.join(candidate,name),path.join(output,'executed-harness',name));
...
const sourceFiles=['R/platform-media-history.R','R/platform-media-review.R','R/platform-media-review-views.R',
  ...
  'www/brohn.css','www/platform-ui.js','www/media-review-ui.js','www/task-plot-ui.js','R/platform-task-plot-views.R'];
...
function authority(mode){const r=spawnSync(runtime.publication_python,[path.join(candidate,'browser-fixture.py'),mode,folder],{windowsHide:true,encoding:'utf8'});assert.equal(r.status,0,r.stderr||r.stdout);return r.stdout;}
...
try{
  authority('before');await start();browser=await chromium.launch({channel:runtime.chrome_channel,headless:true});const context=await browser.newContext({viewport:{width:390,height:844},reducedMotion:'reduce'});
  ...
  const accepted=await exerciseMediaHistory({page,context,output,regular:config.original.regular,gap:config.original.gap,
    openMedia,revokeProject:async active=>authority(active?'revoke':'restore'),restart:async()=>{await stop();await start();},snapshot,referenceBytes});
  const preserved=authority('after');await fs.writeFile(path.join(output,'catalog-preservation.json'),preserved);
...
}finally{authority('restore');if(browser)await browser.close();await stop();await fs.writeFile(path.join(output,'researcher.log'),serviceLog);}
