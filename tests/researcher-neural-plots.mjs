...
async function axe(label,narrow=false){
 ...
 const chart=page.locator('#neural_plot_view svg.brohn-neural-slice:visible');if(await chart.count()){
...
async function htmlCheck(name,expected){const file=await download('Download report',`${name}.html`),html=await fs.readFile(file,'utf8');check(html.includes('Neural responses in context')&&html.includes(expected)&&!/<script[\s>]/.test(html),`${name}: actual standalone HTML carries the supported neural plot without script`);const offline=await context.newPage();await offline.setViewportSize({width:390,height:844});await offline.goto(`file:///${file.replaceAll('\\','/')}`);const layout={plots:await offline.locator('.brohn-neural-plots svg.brohn-neural-slice:visible').count(),maps:await offline.locator('.brohn-neural-plots svg.brohn-neural-map:visible').count(),...(await offline.evaluate(()=>({width:innerWidth,documentWidth:document.documentElement.scrollWidth,hasHeadStyle:!!document.querySelector('head style')})))};await fs.writeFile(path.join(output,`${name}-offline-summary.json`),JSON.stringify(layout,null,2));if(layout.plots!==1||layout.maps!==(name==='morlet'?1:0)||layout.documentWidth>layout.width+1||!layout.hasHeadStyle)exportIssues.push({name,...layout});else check(true,`${name}: standalone HTML retains its head, compact slice, applicable full map and reflows at390px`);await offline.close();}
try{
 ...
 await page.locator('#map_neural_frequencies').fill('10, 20');await page.locator('#map_neural_cycles').fill('3, 3');await select('map_neural_power','Total trial power');await page.locator('#map_neural_summary_start').fill('0');await page.locator('#map_neural_summary_end').fill('.5');await select('map_neural_power_baseline','Divide by baseline power');await page.locator('#map_neural_power_baseline_start').fill('-1');await page.locator('#map_neural_power_baseline_end').fill('-.5');await page.locator('#map_neural_power_floor').fill('0.00000000000000000001');
 await page.locator('#map_neural_baseline_minimum_cycles').fill('4');await page.locator('#map_neural_baseline_rationale').fill('Four cycles for this independently specified stationary sine fixture; not a general EEG recommendation.');await analyse();
 ...
 const baseline=morlet.report.analysis.recordings[0].derived_settings.baseline_support;
 check(baseline.status==='eligible'&&baseline.sample_span_s===.5&&baseline.cycles_at_lowest_frequency===5&&baseline.frequencies.every(x=>x.strictly_before_event),'Actual reviewed baseline retains five observed cycles and finite wavelet separation at every frequency');
 await expect(page.locator('#neural_plot_view')).toContainText('51 samples');await expect(page.locator('#neural_plot_view')).toContainText('not scientific validation');
 ...
 check(channel.series.channel==='Pz'&&channel.series.transformed_power.flat().every(x=>x===null)&&channel.series.itc.flat().every(x=>x===null)&&await page.locator('#neural_plot_view svg.brohn-neural-slice').count()===0,'Actual zero signal has unavailable ratio/phase and no fabricated zero-line chart');
 await select('neural_plot_metric','Power before the power-baseline transform');await expect(page.locator('#neural_plot_view svg.brohn-neural-slice:visible')).toHaveCount(1);check((await json('Download series + provenance','morlet-zero-raw.json')).series.power_uv2.flat().every(x=>x===0),'Raw zero wavelet power remains available separately from its undefined baseline ratio');await axe('morlet-zero-narrow',true);
