...
await import('./fixtures/participant-event-batch-preferred-candidate.js');
const {select, MAX_BYTES, MAX_EVENTS} = globalThis.BrohnEventBatchPreferredCandidate;
let checks = 0;
...
check('Production helper and candidate defaults preserve all exact prior envelope fields',()=>{
  for(const events of [[event(1,'plain'),event(2,'é')],[padded(1,hard),event(2)]]){
    const options=base(events,{maxBytes:hard});assert.deepEqual(select(options),BrohnEventBatch.select(options));
  }
});
check('Default exact-hard-boundary retains original refusal of a following sequence gap',()=>{
  const events=freeze([padded(1,MAX_BYTES),event(3)]);
  rejects(()=>BrohnEventBatch.select(base(events)),'event_sequence_gap');
  rejects(()=>select(base(events)),'event_sequence_gap');
});
console.log(JSON.stringify({passed:checks,candidate_only:true,scope:'Nonloaded candidate pure complete-event selection; actual HTTP comparison is separate.'}));
