...
async function review(page){await expect(page.getByRole('heading',{name:'Review your answers',exact:true})).toBeVisible();await expect(page.locator('#questionnaire-seal')).toBeEnabled();}
async function rest(page){await answer(page,'Original second number',4);await ready(page,'Original optional comment');await page.getByRole('button',{name:'Continue',exact:true}).click();await ready(page,'Original study information');await page.getByRole('button',{name:'Continue',exact:true}).click();await review(page);}
...
try {
  ...
  async function start(setup){const context=await browser.newContext({viewport:{width:1280,height:900}}),page=await context.newPage();lastPage=page;
    ...
    if(setup)await setup(page);await page.goto(`${base}/participant/?token=${config.release.token}`);await page.getByLabel('I have read the study information and agree to take part.',{exact:true}).check();
    const begun=page.waitForResponse(response=>response.url().includes('/api/start/'));await page.getByRole('button',{name:'Start study',exact:true}).click();const run=await(await begun).json();await page.getByRole('button',{name:'Begin',exact:true}).click();await ready(page,'Original first number');return{page,context,run};}
