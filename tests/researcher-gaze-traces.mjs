...
for(const mode of ['serve','worker']){const child=spawn(r,['--vanilla','tests/fixtures/researcher-gaze-traces.R',mode,folder],{env,windowsHide:true});children.push(child);child.stdout.on('data',x=>log+=`${mode}: ${x}`);child.stderr.on('data',x=>log+=`${mode}: ${x}`);}
...
async function scan(label,width){await page.setViewportSize({width,height:1080});await idle();await page.locator('#gaze_trace_plot h2').click();const violations=(await new AxeBuilder({page}).analyze()).violations;
 ...
 scans.push({label,width,violations:violations.length,overflow,small});await fs.writeFile(path.join(output,`${label}-axe.json`),JSON.stringify(violations,null,2));await page.screenshot({path:path.join(output,`${label}.png`),fullPage:true});await page.locator('#gaze_trace_plot').screenshot({path:path.join(output,`${label}-plot.png`)});
...
try{
 ...
 await page.goto(`http://127.0.0.1:${config.port}/`);await button('Studies').click();
 await page.locator(`[data-brohn-event="brohn_open_study"][data-brohn-value='"${config.study_id}"']`).click();await button('Results').click();
 ...
 let saved=await windowData('dirty-full-window.json');check(saved.result.table.identity.participant_id==='person-b'&&saved.result.selected_rows===22,'Exact saved trace belongs to selected person and includes every exposure row');
 ...
 const svgPage=await context.newPage();await svgPage.setContent('<!doctype html><html lang="en"><title>Downloaded trace SVG</title><body style="margin:0">'+svg+'</body></html>');await svgPage.screenshot({path:path.join(output,'exported-trace.png')});await svgPage.close();
 ...
 await fs.writeFile(path.join(output,'acceptance.json'),JSON.stringify({checks,scans,errors,fixture:config,scope:'Original software source; no device or blink-detector qualification'},null,2));console.log(output);
 ...
 await fs.writeFile(path.join(folder,'stop.request'),'owned test shutdown');await browser.close();
 for(const child of children){if(child.exitCode===null)await Promise.race([new Promise(resolve=>child.once('exit',resolve)),new Promise(resolve=>setTimeout(resolve,12000))]);if(child.exitCode===null)child.kill();}
