...
brohn_collection_routes <- function(design) {
  labels <- c(gaze="Eye tracking", questionnaire="Questionnaires", eeg="EEG", eda="EDA", ecg="ECG / HRV", ppg="PPG",
    ...
    voice="Voice", rt="Reaction time", iat="IAT", biat="Brief IAT", aat="Approach / avoidance", maxdiff="Best-worst choices")
  ...
  routes <- lapply(selected, function(measure) {
    ...
    if(measure %in% c("rt","iat","biat","aat")) return(if(measure %in% actual_tasks) row(measure,"In the participant study",
    ...
    if(measure=="facial_expression") return(row(measure,"Expression profile not enabled",
      "This build retains video and supports native face geometry/blendshapes. Interpreted emotion or action-unit scores require their own enabled model/provider profile."))
