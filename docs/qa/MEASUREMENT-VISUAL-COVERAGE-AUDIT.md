...
| **Answers** | `R/platform-questionnaire-explorer-views.R` uses a source-bound index of complete saved typed answers and history. Literal filters, 25/50/100-row pages, unabridged text chunks and exact source detail are available. Whole-report question summaries do not change with answer filters. The separate complete-distribution view adds typed question/scale charts and full exports; see EXPLICIT-DISTRIBUTIONS-ACCEPTANCE.md. |
...
| Whole-recording EDA, `eda-neurokit-highpass/1.0` (`eda.activity`) | Dedicated review aligns cleaned, tonic and phasic µS with exact saved candidate onset/peak/half-recovery markers, candidate pages, complete CSV/JSON/SVG and saved windows. | [Continuous EDA acceptance](EDA-CONTINUOUS-REVIEW-ACCEPTANCE.md):14 native/35 R/21 browser checks and five scans, exact independent marker geometry and unchanged restart. Actual5,000-candidate publication,11 background-verification and4 authority checks have separate receipts. Missing endpoints, gaps and conditional denominators remain explicit. No raw reconstruction, absolute threshold or stimulus attribution is inferred. |
| Event EDA, `eda-event-highpass/1.0` (`eda.activity`) | **Signal** continuous cleaned/tonic/phasic µS plus candidate-event artifact; **Report** adds readable baseline/response/SCR support tables and exact eligible event/channel denominators. Event review now overlays authored baseline/response/latency/recovery windows and saved selected/candidate SCR markers on the complete retained trace. | Nonresponse, unsupported amplitude, overlap and actual sample gaps remain distinct. [Event review acceptance](EDA-EVENT-REVIEW-ACCEPTANCE.md):27 R/11 reader/22 browser/38 independent checks and six clean inspected scans; complete selected samples, source events/features/support, SVG and restart verified. No window-based reanalysis or inferred overlap attribution. |
...
| Respiration, `respiration-displacement-khodadad/1.0` (`respiration.cycles`) | Dedicated saved review overlays complete cycle phases and extrema on normalized cleaned displacement/volume, with exact cycle/sample tables, numerical exports and saved windows. Units, declared inspiration polarity and normalization remain explicit. | [Connected acceptance](RESPIRATION-REVIEW-ACCEPTANCE.md):16 reader/27 domain/21 uninterrupted browser checks and five scans; actual import, scoring, paging, exports and restart. Final shared respiration/EMG history and axis checks add12 browser assertions/two scans with zero new jobs. Raw signed source is not reconstructed; airflow remains outside this phase recipe. |
| Surface EMG, `emg-butterworth-rms/1.0` (`emg.activation`) | Dedicated review exposes converted pre-cleaning input, cleaned voltage and RMS in µV, with saved optional threshold-burst overlays, exact tables/exports and saved windows. Report frequency summaries remain separate from these traces. | [Connected acceptance](EMG-REVIEW-ACCEPTANCE.md):13 reader/31 domain/24 browser checks and six scans across11 successful jobs. Complete mV-to-µV input, explicit8µV threshold, gaps/partial/empty states and restart preserve originals. Historical artifacts without input disclose its absence. Current axes/history have a separate accepted receipt. No full EMG spectrum, MVC-normalized, facial/startle or fatigue interpretation is inferred. |
...
| Audio, `audio-praat-acoustics/1.0` (`audio.acoustics`, acoustic subset) | **Signal** saved frame RMS in FS, spectral centroid and pitch in Hz. Source review adds exact selected-channel waveform envelopes and periodic-Hann density spectrogram in FS²/Hz, with complete samples/cells and separate display aggregation. | [Audio review acceptance](AUDIO-SOURCE-REVIEW-ACCEPTANCE.md):34 worker/28 domain checks,21 browser checks/four scans plus separate restart and actual SVG-label bounds receipts. Pitch null/unvoiced, zero energy and short spectral windows stay distinct. Cold reopening measured8.23s. No speech transcript, calibrated SPL, emotion or attention interpretation; video audio derivation now has separate connected acceptance in VIDEO-AUDIO-EXTRACTION-ACCEPTANCE.md. |
...
| Twelve questionnaire types plus branching/revision (`questionnaire.items`, `questionnaire.flow`) | **Answers** complete typed explorer and saved whole-report counts; numeric means only where the saved analysis supplies them. Zero/false/text/null, omissions, not displayed, acknowledged information and invalidations remain distinct. | [Explorer acceptance](QUESTIONNAIRE-EXPLORER-ACCEPTANCE.md): 77 pure/59 storage/40 Shiny; 14 browser assertions/five scans with a subsequent layout-only retake. Complete exports, long Unicode and history tested. Complete source-bound distributions now have29 focused/14 worker/11 browser checks/six scans and independent exports (EXPLICIT-DISTRIBUTIONS-ACCEPTANCE.md). Categorical, numeric and compound values retain different display semantics. |
| Scale scoring `explicit-questionnaire-scale/1.0` (`questionnaire.scales`) | **Report** scale coverage and first 30 assessment rows; complete scale-score CSV and full item-evidence JSON. Saved key, bounds, reverse scoring, missing/prorated status and separate assessments remain explicit. | Actual scale/revision/export journeys are recorded in researcher QA. The separate complete-distribution explorer now reads all saved assessments with counts by records/assessments/linked people and complete exports; see EXPLICIT-DISTRIBUTIONS-ACCEPTANCE.md. Repeated-assessment longitudinal modelling is still separate. No reliability/normative interpretation is implied. |
...
| Declared paired contrasts and `multimodal-explicit-crosswalk/1.0-draft` (`multimodal.analysis`) | **Report** numerical contrast cards with unit, retained people/paired sessions, uncertainty/support and source coverage; original metric observations export as CSV with full JSON/HTML. | Actual controlled/multimodal/peripheral browser routes are documented in researcher QA. Complete paired-person figures now have32 domain/28 browser checks/five scans and13 independent export checks (PAIRED-RESULTS-ACCEPTANCE.md). Linked same-recording declared-clock signal/event review also passes its own scoped journey (LINKED-REVIEW-ACCEPTANCE.md); it does not establish physical alignment or synchronized media replay. Reviewed identity mapping permits parallel measure comparison, not inferred temporal synchronization. |
...
larger capacity and wider synchronized multimodal/media replay. Whole-recording
EDA candidate overlays, EMG input/envelope/burst review, respiration cycle/phase overlays and
video audio derivation now have scoped connected acceptance in the ledger;
they do not establish physical synchronization or device validity.
Event-EDA overlays, source audio waveform/spectrogram, questionnaire/scale
distributions, paired-person figures and linked same-source signal/event review now have scoped
acceptance in the integrated workflow ledger. Exact values and an accessible generic chart do not replace
...
separate evidence. Repeated canonical traversal now has a qualified bounded-hash fix on the recorded
ECG journey (SIGNAL-REVIEW-RESPONSIVENESS.md); other large reports and concurrent
workloads still need profiling with original source authority preserved.
