...
| --- | --- | --- | --- | --- |
| **P0 Study design → runnable plan** | H10, V10; [Study Builder](https://imotions.com/products/imotions-lab/features/study-builder/) documents media, blocks, study flows, sensor selection and live markers. | Start from blank/template; add baseline, media, survey and task; batch-edit, assign two flows, preview actual participant layout, freeze and run; edit a reusable source without changing the frozen run. | Libraries, clones, templates, portable designs, branching, five task profiles and [question illustrations](QUESTION-ILLUSTRATIONS-ACCEPTANCE.md) have connected evidence. Guided Overview, welcome and saved material editors now have dedicated browser/worker acceptance. One multimodal scenario must show all transitions without manual database/script intervention. `design.protocols`, BWP03–05/15. | Representative researchers for observed completion; protocol materials for additional task packs. |
| **P0 Device setup → live quality → collection** | H81, H11; [LSL support](https://imotions.com/products/imotions-lab/developers/lsl-support/) explicitly depends on an available LSL-capable device adapter. | Choose named streams, inspect units/rates/quality, associate participant and frozen plan, calibrate where required, start/stop visibly and seal originals; dropout/reset/disconnect have explicit outcomes. | [Local acquisition](../operations/LOCAL-ACQUISITION.md) has explicit selection, original archives and recovery; [guarded publication and monitoring](ACQUISITION-PUBLICATION-AND-MONITORING.md), [15-family acquisition criteria](ACQUISITION-WINDOW-AND-CHECKS.md) and [reusable equipment setups](EQUIPMENT-SETUP-REUSE.md) now have manager/replay/browser acceptance. This is transport and declared-check evidence, not physical-device or scientific signal qualification. `centralization.recordings`, BWP06/16. | Named physical devices, entitled SDKs and reference/calibration equipment. |
...
| **P0 Annotations → reusable analysis windows** | H92, V2, R26. | Add/edit marker and interval by keyboard/mouse; save author/history; define an event-relative window once; apply across chosen participants/stimuli; handle overlapping, absent and repeated events; recompute a new report without altering the old one. | Event-related EEG/EDA and pinned comparison plans exist. [Saved signal intervals](SIGNAL-INTERVAL-ACCEPTANCE.md) and [explicit-anchor reuse across recordings](SIGNAL-INTERVAL-REUSE-ACCEPTANCE.md) now have actual worker/browser acceptance. Automatic common event rules across media/modalities, measured synchronization and drift handling remain open. BWP07–09/13/15. | Independent event/window fixtures; domain review of each interpretation. |
