...
function Read-BrohnLocalConfiguration {
    ...
    if (@($required | Where-Object { $_ -notin $keys }).Count -or @($keys | Where-Object { $_ -notin ($required + @('checked_at')) }).Count -or
    ...
    foreach ($property in $value.scientific.PSObject.Properties) {
        if ($property.Name -cnotin @('methods','acquisition','vision-audio','segmentation')) { throw "Unknown scientific profile: $($property.Name)" }
